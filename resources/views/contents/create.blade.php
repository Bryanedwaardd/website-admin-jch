...
@section('content')
    <h2 class="text-2xl font-semibold text-gray-800 mb-4">Buat Konten Baru</h2>

    <form action="{{ route('contents.store') }}" method="POST" class="bg-white p-6 rounded-lg shadow-md">
        @csrf

        <div class="mb-4">
            <label for="EventName" class="block text-gray-700">Nama Konten</label>
        ...
        <div class="mb-4">
            ...
            <textarea name="Description" id="Description" class="w-full p-2 border border-gray-300 rounded" required>{{ old('Description') }}</textarea>
        </div>
            ...
            <select name="Platform" id="Platform" class="w-full p-2 border border-gray-300 rounded" required>
                @foreach(App\Enums\PlatformEnum::cases() as $platform)
                    <option value="{{ $platform->value }}" 
                            @if(old('Platform') === $platform->value) selected @endif>
